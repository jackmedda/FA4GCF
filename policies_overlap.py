...
def perturbation_sampler_kwargs_gen(conf):
    for pol in (user_policies + item_policies):
        ...
        for conf_pol in new_conf['explainer_policies']:
            new_conf['explainer_policies'][conf_pol] = False
        new_conf['explainer_policies']['gradient_deactivation_constraint'] = True
        new_conf['explainer_policies']['group_deletion_constraint'] = True
        new_conf['explainer_policies'][pol] = True
        ...
        interaction_recency_constraint = new_conf['explainer_policies']['interaction_recency_constraint']
        interaction_recency_constraint_ratio = new_conf['interaction_recency_constraint_ratio']
...
if __name__ == "__main__":
    ...
    config = Config(dataset=args.dataset_name.lower(), model=args.model_name, config_file_list=[dataset_config_file])
    dataset_exp_config_path = os.path.join(current_path, 'config', 'explainer', f"{args.dataset_name.lower()}_explainer.yaml")
    exp_config_dict = config.update_base_explainer(dataset_exp_config_path)

    config, model, dataset, train_data, valid_data, test_data = load_data_and_model(
        model_file, explainer_config=exp_config_dict
    )
    ...
    _, rec_model_topk = perturbation_trainer._get_model_score_data(user_list, valid_data, config["cf_topk"])
    ...
    for curr_policy, sampler_kwargs in perturbation_sampler_kwargs_gen(config):
    ...
    for pol_type in ['user','item']:
        ...
        with open(file_name + ".csv", 'w') as file:
            file.write(f"{dataset.field2id_token[perturbation_trainer.sensitive_attribute]}      adv_group: {perturbation_trainer.adv_group}\n")
